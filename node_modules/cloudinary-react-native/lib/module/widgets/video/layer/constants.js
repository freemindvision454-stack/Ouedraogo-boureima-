...
export const TOP_BUTTON_SIZE = Platform.select({
  ios: 44,
  android: 48
});
export const CENTER_PLAY_BUTTON_SIZE = 72; // Center button size
export const BOTTOM_BUTTON_SIZE = Platform.select({
  ios: 40,
  android: 44
});
...
export const BOTTOM_CONTROLS_PADDING = Platform.select({
  ios: 15,
  android: 12
});
export const BOTTOM_CONTROLS_PADDING_LANDSCAPE = Platform.select({
  ios: 10,
  android: 8
});
export const SEEKBAR_ALIGNMENT_OFFSET = Platform.select({
  ios: 16,
  android: 14
}); // Platform-specific alignment
export const SEEKBAR_ALIGNMENT_OFFSET_LANDSCAPE = Platform.select({
  ios: 12,
  android: 10
});
...
export const SE_BUTTON_BOTTOM_OFFSET = Platform.select({
  ios: 32,
  android: 28
}); // Platform-specific bottom offset
export const SE_BUTTON_BOTTOM_OFFSET_LANDSCAPE = Platform.select({
  ios: 20,
  android: 18
});

// Button spacing constants for multiple buttons in the same position
export const BUTTON_SPACING = Platform.select({
  ios: 8,
  android: 6
}); // Space between buttons
export const BUTTON_MARGIN = Platform.select({
  ios: 4,
  android: 3
}); // Margin from edge
