 ...
 */
export const handleDefaultShare = async cldVideo => {
  try {
 ...
 */
export const validateSeekPosition = (seekPosition, duration, bufferMs = 100) => {
 ...
 */
export const hasSignificantPositionDifference = (position1, position2, threshold = 100) => {
 ...
 */
export const calculateSeekProgress = (touchX, seekbarWidth) => {
 ...
 */
export const isHLSVideo = videoUrl => {
 ...
 */
export const parseHLSManifest = async manifestUrl => {
  try {
 ...
 */
export const parseHLSQualityLevels = async manifestUrl => {
  try {
    const response = await fetch(manifestUrl);
    if (!response.ok) {
 ...
 */
export const getVideoUrl = (videoUrl, cldVideo) => {
...
const TOP_BUTTON_SIZE = Platform.select({
  ios: 44,
  android: 48
});
const BUTTON_SPACING = Platform.select({
  ios: 8,
  android: 6
});
const BUTTON_MARGIN = Platform.select({
  ios: 4,
  android: 3
});
...
const BOTTOM_CONTROLS_PADDING = Platform.select({
  ios: 15,
  android: 12
});
const BOTTOM_CONTROLS_PADDING_LANDSCAPE = Platform.select({
  ios: 10,
  android: 8
});
const SEEKBAR_ALIGNMENT_OFFSET = Platform.select({
  ios: 16,
  android: 14
});
const SEEKBAR_ALIGNMENT_OFFSET_LANDSCAPE = Platform.select({
  ios: 12,
  android: 10
});
const SE_BUTTON_BOTTOM_OFFSET = Platform.select({
  ios: 32,
  android: 28
});
const SE_BUTTON_BOTTOM_OFFSET_LANDSCAPE = Platform.select({
  ios: 20,
  android: 18
});
 ...
 */
export const getTopPadding = (isLandscape = false) => {
  if (Platform.OS === 'ios') {
 ...
 */
export const getBottomControlsPadding = (isLandscape = false) => {
 ...
 */
export const getSeekbarAlignmentOffset = (isLandscape = false) => {
 ...
 */
export const getSEButtonBottomOffset = (isLandscape = false) => {
 ...
 */
export const calculateButtonPosition = (position, index, totalButtons, isLandscape = false, layoutDirection = ButtonLayoutDirection.VERTICAL) => {
