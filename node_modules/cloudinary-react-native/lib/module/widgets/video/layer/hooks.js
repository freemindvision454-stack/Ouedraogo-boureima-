...
import { CONTROLS_AUTO_HIDE_DELAY, CONTROLS_FADE_DURATION, SEEK_DEBOUNCE_DELAY, SEEK_POSITION_THRESHOLD, SEEK_POSITION_TOLERANCE } from './constants';
...
export const useVideoControlsVisibility = () => {
  ...
  const autoHideTimeoutId = React.useRef(null);
  ...
  const startAutoHideTimer = React.useCallback((isControlsVisible, setIsControlsVisible) => {
    clearAutoHideTimer();
  ...
  const toggleControls = React.useCallback((isControlsVisible, setIsControlsVisible) => {
...
export const useVideoSeeking = () => {
  const seekTimeoutId = React.useRef(null);
  ...
  const handleSeekMove = React.useCallback((evt, seekbarRef, status) => {
  ...
  const handleSeekEnd = React.useCallback((evt, seekbarRef, status, videoRef) => {
    if (seekbarRef.current && status) {
      const touchPageX = evt.nativeEvent.pageX;
      return new Promise(resolve => {
        seekbarRef.current.measure((_x, _y, width, _height, pageX, _pageY) => {
  ...
  const getProgress = React.useCallback((status, isSeeking, seekingPosition, isSeekingComplete, lastSeekPosition) => {
    if (!status) return 0;
  ...
  const getCurrentPosition = React.useCallback((status, isSeeking, seekingPosition, isSeekingComplete, lastSeekPosition) => {
    if (!status) return 0;
    const currentVideoPosition = status.positionMillis || 0;
    if (isSeeking) {
  ...
  const handleStatusUpdate = React.useCallback((s, isSeekingComplete, lastSeekPosition, setStatus, setSeekingState) => {
    if (s.isLoaded) {
      if (isSeekingComplete && lastSeekPosition > 0) {
