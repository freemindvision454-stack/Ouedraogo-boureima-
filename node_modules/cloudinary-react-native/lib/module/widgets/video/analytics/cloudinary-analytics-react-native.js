...
import { createRegularVideoViewEndEvent, createLiveStreamViewStartEvent, createLiveStreamViewEndEvent, createRegularVideoViewStartEvent, prepareEvents } from './utils/events';
...
export const connectCloudinaryAnalytics = (videoRef, mainOptions = {}) => {
  throwErrorIfInvalid(mainOptionsValidator(mainOptions), 'Cloudinary video analytics requires proper options object');
  ...
  const startManualTracking = (metadata, options = {}) => {
    ...
    throwErrorIfInvalid(metadataValidator(metadata), 'Cloudinary video analytics manual tracking called without necessary data');
    throwErrorIfInvalid(trackingOptionsValidator(options), 'Cloudinary video analytics manual tracking called with invalid options');
    clearVideoTracking();
  ...
  const _startManualTrackingRegularVideo = (metadata, options) => {
    ...
    const appStateRemoval = createAppStateTracker({
      onAppForeground: () => {
        ...
        videoViewEventCollector.addEvent(createRegularVideoViewStartEvent({
  ...
  const _startManualTrackingLiveStream = (metadata, options) => {
    ...
    const appStateRemoval = createAppStateTracker({
      onAppForeground: () => {
        viewId.regenerateValue();
        sendLiveStreamEvent(createLiveStreamViewStartEvent({
          liveStreamData
        }, options));
      },
      onAppBackground: () => {
        sendLiveStreamEvent(createLiveStreamViewEndEvent({
          liveStreamData
        }, options));
      }
    });
  ...
  const startAutoTracking = (options = {}) => {
    ...
    throwErrorIfInvalid(trackingOptionsValidator(options), 'Cloudinary video analytics auto tracking called with invalid options');
    const sendData = data => {
      return sendRequest(CLD_ANALYTICS_ENDPOINT.default, data);
    };
    const videoViewEventCollector = createEventsCollector();
