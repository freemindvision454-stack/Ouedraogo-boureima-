...
exports.triggerExpoAVEvent = exports.processExpoAVStatus = exports.expoAVVideoPlayerAdapter = void 0;
...
const createBaseOnEventListener = (videoRef, eventName, callback) => {
  ...
  return () => {
    var _videoRef$_cloudinary;
    const callbacks = (_videoRef$_cloudinary = videoRef._cloudinaryEventCallbacks) === null || _videoRef$_cloudinary === void 0 ? void 0 : _videoRef$_cloudinary[eventName];
    if (callbacks) {
...
const triggerExpoAVEvent = (videoRef, eventName, eventData) => {
  var _videoRef$_cloudinary2;
  const callbacks = (_videoRef$_cloudinary2 = videoRef._cloudinaryEventCallbacks) === null || _videoRef$_cloudinary2 === void 0 ? void 0 : _videoRef$_cloudinary2[eventName];
  if (callbacks) {
...
exports.triggerExpoAVEvent = triggerExpoAVEvent;
const processExpoAVStatus = (videoRef, status, previousStatus = {}) => {
  if (!status.isLoaded) {
    // Handle error state
    if (status.error) {
      triggerExpoAVEvent(videoRef, 'error', {
        error: status.error
      });
    }
    return;
...
exports.processExpoAVStatus = processExpoAVStatus;
const expoAVVideoPlayerAdapter = videoRef => ({
  ...
  getCurrentSrc: () => {
    var _videoRef$_currentSta;
    return ((_videoRef$_currentSta = videoRef._currentStatus) === null || _videoRef$_currentSta === void 0 ? void 0 : _videoRef$_currentSta.uri) || '';
  },
  getCurrentTime: () => {
    var _videoRef$_currentSta2;
    return (((_videoRef$_currentSta2 = videoRef._currentStatus) === null || _videoRef$_currentSta2 === void 0 ? void 0 : _videoRef$_currentSta2.positionMillis) || 0) / 1000;
  },
  getReadyState: () => {
    const status = videoRef._currentStatus;
    if (!(status !== null && status !== void 0 && status.isLoaded)) return 0;
    return 4; // HAVE_ENOUGH_DATA
  },
  getDuration: () => {
    var _videoRef$_currentSta3;
    return (((_videoRef$_currentSta3 = videoRef._currentStatus) === null || _videoRef$_currentSta3 === void 0 ? void 0 : _videoRef$_currentSta3.durationMillis) || 0) / 1000;
  }
});
exports.expoAVVideoPlayerAdapter = expoAVVideoPlayerAdapter;
//# sourceMappingURL=expoAVVideoPlayerAdapter.js.map