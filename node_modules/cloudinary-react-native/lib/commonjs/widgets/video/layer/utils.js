...
exports.validateSeekPosition = exports.parseHLSQualityLevels = exports.parseHLSManifest = exports.isLandscapeOrientation = exports.isHLSVideo = exports.hasSignificantPositionDifference = exports.handleDefaultShare = exports.getVideoUrl = exports.getTopPadding = exports.getSeekbarAlignmentOffset = exports.getSEButtonBottomOffset = exports.getBottomControlsPadding = exports.formatTime = exports.calculateSeekProgress = exports.calculateButtonPosition = void 0;
...
const handleDefaultShare = async cldVideo => {
  try {
    // Create analytics object with feature 'G' for CLDVideoLayer
    const videoLayerAnalytics = {
      ..._SDKAnalyticsConstants.SDKAnalyticsConstants,
      feature: 'G'
    };
    let videoUrl;
    try {
      videoUrl = cldVideo.toURL({
        trackedAnalytics: videoLayerAnalytics
      });
    ...
    await _reactNative.Share.share({
      message: _reactNative.Platform.OS === 'ios' ? '' : videoUrl,
      url: videoUrl
    });
  } catch (error) {
 ...
 */
exports.handleDefaultShare = handleDefaultShare;
const validateSeekPosition = (seekPosition, duration, bufferMs = 100) => {
 ...
 */
exports.validateSeekPosition = validateSeekPosition;
const hasSignificantPositionDifference = (position1, position2, threshold = 100) => {
 ...
 */
exports.hasSignificantPositionDifference = hasSignificantPositionDifference;
const calculateSeekProgress = (touchX, seekbarWidth) => {
 ...
 */
exports.calculateSeekProgress = calculateSeekProgress;
 ...
 */
exports.isHLSVideo = isHLSVideo;
const parseHLSManifest = async manifestUrl => {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line !== null && line !== void 0 && line.startsWith('#EXT-X-MEDIA:') && line.includes('TYPE=SUBTITLES')) {
...
const parseHLSQualityLevels = async manifestUrl => {
  try {
    const response = await fetch(manifestUrl);
    if (!response.ok) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line !== null && line !== void 0 && line.startsWith('#EXT-X-STREAM-INF:')) {
        var _lines;
        const attributes = parseM3U8Attributes(line);
        const streamUrl = (_lines = lines[i + 1]) === null || _lines === void 0 ? void 0 : _lines.trim(); // Next line contains the stream URL
 ...
 */
exports.parseHLSQualityLevels = parseHLSQualityLevels;
 ...
 */
const getVideoUrl = (videoUrl, cldVideo) => {
...
const TOP_BUTTON_SIZE = _reactNative.Platform.select({
  ios: 44,
  android: 48
});
const BUTTON_SPACING = _reactNative.Platform.select({
  ios: 8,
  android: 6
});
const BUTTON_MARGIN = _reactNative.Platform.select({
  ios: 4,
  android: 3
});
...
const BOTTOM_CONTROLS_PADDING = _reactNative.Platform.select({
  ios: 15,
  android: 12
});
const BOTTOM_CONTROLS_PADDING_LANDSCAPE = _reactNative.Platform.select({
  ios: 10,
  android: 8
});
const SEEKBAR_ALIGNMENT_OFFSET = _reactNative.Platform.select({
  ios: 16,
  android: 14
});
const SEEKBAR_ALIGNMENT_OFFSET_LANDSCAPE = _reactNative.Platform.select({
  ios: 12,
  android: 10
});
const SE_BUTTON_BOTTOM_OFFSET = _reactNative.Platform.select({
  ios: 32,
  android: 28
});
const SE_BUTTON_BOTTOM_OFFSET_LANDSCAPE = _reactNative.Platform.select({
  ios: 20,
  android: 18
});
 ...
 */
exports.isLandscapeOrientation = isLandscapeOrientation;
const getTopPadding = (isLandscape = false) => {
...
const getBottomControlsPadding = (isLandscape = false) => {
 ...
 */
exports.getBottomControlsPadding = getBottomControlsPadding;
const getSeekbarAlignmentOffset = (isLandscape = false) => {
 ...
 */
exports.getSeekbarAlignmentOffset = getSeekbarAlignmentOffset;
const getSEButtonBottomOffset = (isLandscape = false) => {
 ...
 */
exports.getSEButtonBottomOffset = getSEButtonBottomOffset;
const calculateButtonPosition = (position, index, totalButtons, isLandscape = false, layoutDirection = _types.ButtonLayoutDirection.VERTICAL) => {
  ...
  const getVerticalLayout = (baseOffset = 0) => {
...
exports.calculateButtonPosition = calculateButtonPosition;
